...
# Josef's Regel categorization + tradeable/non-tradeable split live in the shared
# core module — imported here so the snapshot stays consistent with the rest of the app.
from core.portfolio_stability import (
    NON_TRADEABLE_CLASSES as _NON_TRADEABLE_CLASSES,
    JOSEF_CATEGORY as _JOSEF_CATEGORY,
    compute_josef_allocation,  # re-exported for backwards-compatible imports
)
...
SYSTEM_PROMPT = """You are a long-term investment sounding board — NOT a rebalancing optimizer.
You receive a snapshot of the user's portfolio with weights, performance, allocation context, and per-position agent signals (thesis / fundamental / consensus-gap verdicts).

Core stance (read first):
- The default, and often correct, answer is "hold — do nothing." Inaction is a legitimate recommendation. Never manufacture activity to look useful.
- NEVER suggest small cosmetic weight tweaks ("trim X by 3%, add 2% to Y") to nudge the portfolio toward a target allocation. The user finds that noise and explicitly does not want it.
- Rebalancing happens primarily through NEW INFLOWS, not by selling winners. Treat selling as a high-bar action that needs a concrete reason.
- The allocation/Josef breakdown in the snapshot is context for spotting real concentration — it is NOT a target you should nag the user toward.

Only recommend trimming or selling a position when there is a concrete trigger:
- the investment thesis is broken (thesis: gefaehrdet)
- it has become clearly overvalued (fundamental: überbewertet)
- the consensus gap has closed (gap: eingeholt) — the original edge is gone
- genuine, outsized cluster/concentration risk (not mild drift from a target)

Swap decisions (the main use case):
- The user often wants to buy something new but has no fresh cash — the purchase must be funded by trimming or selling an existing position.
- When the user names a buy candidate (e.g. "I want to buy Alphabet"), rank the WEAKEST existing positions as funding sources using the triggers above, and name 1–3 concrete candidates with the reasoning behind each. Be explicit this is a trade-off, not free money.
- If nothing is genuinely weak, say so plainly — and suggest funding the buy from new inflows or the cash position rather than forcing a sale.

Style:
- Recommendations only — you cannot execute trades.
- Be decisive about discrete buy / sell / swap / hold calls. Do not hedge into vague percentage adjustments.
- Be cost-aware. Keep output structured and scannable. Respect the chosen strategy lens.
- Positions marked [AUSGESCHLOSSEN] are excluded — make no buy/sell recommendation for them.
- Non-tradeable positions (cash, fixed-term deposits, real estate) are context only.
...
class RebalanceAgent:
    ...
    async def start_session(
        ...
        user_message = user_context.strip() if user_context.strip() else (
            "Portfolio-Review: Wo sind echte Red Flags und die schwächsten Glieder? "
            "Nur handeln bei klarem Auslöser (These gebrochen, überbewertet, Lücke eingeholt, "
            "echtes Klumpenrisiko) — wenn nichts dringend ist, sag das. Keine kosmetischen %-Tweaks."
        )
    ...
    def _build_portfolio_context(
        ...
        fund_v      = self._analyses.get_latest_bulk(all_ids, "fundamental_analyzer")
        gap_v       = self._analyses.get_latest_bulk(all_ids, "consensus_gap")
        fund_v_wl   = self._analyses.get_latest_bulk([w.id for w in watchlist if w.id], "fundamental_analyzer")
        ...
        for pos in tradeable:
            ...
            excluded_tag = " **[AUSGESCHLOSSEN]**" if pos.analysis_excluded else ""
