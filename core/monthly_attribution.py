...
def compute_monthly_attribution(
    ...
    Quantity source: when ``wealth_repo`` is given, the *actual* quantity held at the
    start of the period is read from the nearest wealth snapshot's ``holdings`` (so a
    later quantity change no longer distorts a past period). Falls back to today's
    ``v.quantity`` when no snapshot carries composition near the period start.
    ...
    # Quantity held at the start of the period (forward-only; None → today's qty).
    # Snapshot holdings are keyed by ticker — the resolver splits a ticker held in
    # several depots back across its positions.
    start_qty_map = wealth_repo.holdings_near_date(month_start) if wealth_repo is not None else None
    ...
    for v in portfolio_vals:
        sv = _get_start_value_monthly(
            market_repo, v, prev_month_start, prev_month_end, period_start,
    ...
    for v in portfolio_vals:
        ...
        # Not held during this month at all — a later purchase must not produce a
        # contribution in months before it was bought.
...
def _get_start_value_monthly(market_repo, v, prev_month_start: str, prev_month_end: str, period_start: date, start_qty=None, period_end: Optional[date] = None) -> Optional[float]:
