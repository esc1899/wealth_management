...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    for stmt in [
        ...
        """CREATE TABLE IF NOT EXISTS cowork_research_entries (
            id                INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        "CREATE INDEX IF NOT EXISTS idx_cowork_entries_date ON cowork_research_entries(date)",
        """CREATE TABLE IF NOT EXISTS cowork_watchlist_suggestions (
            id                  INTEGER PRIMARY KEY AUTOINCREMENT,
...
def migrate_db(conn: sqlite3.Connection) -> None:
    ...
    if "web_search_requests" not in existing_usage:
        conn.execute("ALTER TABLE llm_usage ADD COLUMN web_search_requests INTEGER")
    if "generation_id" not in existing_usage:
        conn.execute("ALTER TABLE llm_usage ADD COLUMN generation_id TEXT")
    if "actual_cost_usd" not in existing_usage:
        conn.execute("ALTER TABLE llm_usage ADD COLUMN actual_cost_usd REAL")
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS consensus_gap_sessions (
        id            INTEGER PRIMARY KEY AUTOINCREMENT,
        position_id   INTEGER NOT NULL,
        ticker        TEXT,
        position_name TEXT NOT NULL,
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS consensus_gap_messages (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
        session_id INTEGER NOT NULL REFERENCES consensus_gap_sessions(id),
        role       TEXT NOT NULL,
        content    TEXT NOT NULL,
        created_at TEXT NOT NULL
    )""")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_consensus_gap_messages_session ON consensus_gap_messages(session_id)")

    conn.execute("""CREATE TABLE IF NOT EXISTS capital_allocator_sessions (
        id            INTEGER PRIMARY KEY AUTOINCREMENT,
        position_id   INTEGER NOT NULL,
        ticker        TEXT,
        position_name TEXT NOT NULL,
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS capital_allocator_messages (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
        session_id INTEGER NOT NULL REFERENCES capital_allocator_sessions(id),
        role       TEXT NOT NULL,
        content    TEXT NOT NULL,
        created_at TEXT NOT NULL
    )""")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_capital_allocator_messages_session ON capital_allocator_messages(session_id)")
    ...
    existing_prices = {row[1] for row in conn.execute("PRAGMA table_info(current_prices)")}
